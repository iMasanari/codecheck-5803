...
function getUri(keyword: string, releaseDate: string, start: number) {
    let q = encodeURIComponent(`Body:${keyword} AND ReleaseDate:${releaseDate}`)

    return `http://54.92.123.84/search?q=${q}&rows=100&start=${start}&wt=json&ackey=${ackey}`
}

function sendRequest(url: string) {
    return new Promise<Json>(done => {
...
async function main(argv: string[]) {
    ...
    const docsList = await Promise.all(list.map(async keyword => {
        const {response} = await sendRequest(getUri(keyword, `[${start} TO ${end}]`, 0))

        const numFound = +response.result.numFound
        let docs = response.result.doc

        // 追加読み込み
        const promises = []
        for (let i = 1; (i - 1) * 100 <= numFound; ++i) {
            const promise = sendRequest(getUri(keyword, `[${start} TO ${end}]`, i * 100)).then(({ response }) => {
    ...
    const coefficients = countsList.map(value1 =>
        countsList.map(value2 => {
